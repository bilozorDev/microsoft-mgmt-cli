...
export class PowerShellSession {
  ...
  async ensureGraphConnected(): Promise<void> {
    ...
    const { error } = await this.runCommand(
      'Connect-MgGraph -Scopes "User.ReadWrite.All","Organization.Read.All","Directory.ReadWrite.All","Group.ReadWrite.All","GroupMember.ReadWrite.All","AuditLog.Read.All" -NoWelcome',
    );
    if (error) {
